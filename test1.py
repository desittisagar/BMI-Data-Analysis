...
f1 = open("bmi_men.csv", "r")
men=f1.read()
men1=men.split("\n")[1:-1]
men_list=[]
for i in range(len(men1)):
    x=men1[i].split(",")
    for j in range(1, len(x)):
        x[j] = float(x[j])
    men_list.append(x)
for i in range(len(men_list)):
    key, value = men_list[i][0], men_list[i][1:]
    bmi_men_dict[key] = value
   
f2 = open("bmi_women.csv", "r")
women=f2.read()
women1=women.split("\n")[1:-1]


women_list=[]
for i in range(len(women1)):
    x=women1[i].split(",")
    for j in range(1, len(x)):
        x[j] = float(x[j])
    women_list.append(x)
for i in range(len(women_list)):
    key, value = women_list[i][0], women_list[i][1:]
    bmi_women_dict[key] = value
...
for i, j in bmi_men_dict.items():
    x1 = []
    for x, y in bmi_women_dict.items():
        if i == x:    
            bmi_all[i]=res_list = [round((j[k] + y[k])/2,2) for k in range(29)]
            #print(res_list)
            # print(i)
            # print(res_list)
            x1.append(i)
...
while True:
    year = (input("Select a year to find statistics (1980 to 2008): "))
    if year.isdigit() and int(year)>=1980 and int(year)<=2008:
        pos = int(year) - 1980
        min_val = min(list(try_bmi[:][pos]))
        max_val = max(list(try_bmi[:][pos]))
        temp_list = list(try_bmi[:][pos])
        temp_list.sort()
        min_country = ''
        max_country = ''

        for i in range(0,186):
            if min_val == try_bmi[i][pos]:
                min_country = try_bmi[i][0]
            if max_val == try_bmi[i][pos]:
                max_country = try_bmi[i][0]

        print(f"\nIn {year}, countries with minimum and maximum BMI values were '{min_country}' and '{max_country}' and Median BMI value in {year} was {round(((temp_list[92]+temp_list[93])/2.000),5)}")
        print("\n\n")
        break
    else:
